...
def get_keyword_index(keyword):
    """Get the index of a keyword in the ALL_KEYWORDS list."""
    try:
        return ALL_KEYWORDS.index(keyword)
    except ValueError:
        print(f"❌ ERROR: Keyword '{keyword}' not found in ALL_KEYWORDS list!")
        return None
...
def main():
    ...
    data_store = {kw: {c: {} for c in TARGET_COUNTRIES} for kw in VALIDATION_KEYWORDS}
            ...
            for country, filepath in file_map.items():
                try:
                    ...
                    if 'similarity_matrix' in data:
                        ...
                        print(f"    ⚠ No similarity_matrix in {filepath.name}, using raw embeddings")
                        embeds = data['embeddings']
                        if len(embeds) == 0:
                            continue
                        # This would require re-encoding, which we want to avoid
                        # Skip this file
                        continue
